...
const TutorialBreeding = function(storage, overlay) {
    ...
    overlay.setText(window.window.language.get(this.LANG_MOVE_FISH));
};
...
TutorialBreeding.prototype.targetRiverFish = function(constellation) {
    ...
    for (const fish of constellation.big.fish) {
    ...
    switch (this.phase) {
        ...
        case this.PHASE_DROP_FISH:
            if (!koi.mover.move) {
                if (koi.constellation.small.fish.length === 0) {
                    this.overlay.setText(window.language.get(this.LANG_TO_POND_1));

                    this.pointToSmallPond(koi);
                    this.advance();
                }
                else {
                    this.overlay.setText(window.language.get(this.LANG_TO_POND_2));
        ...
        case this.PHASE_TO_POND_2:
            if (koi.constellation.small.fish.length === 2) {
                this.overlay.setText(window.language.get(this.LANG_BREED_WAIT));
        ...
        case this.PHASE_BREED_WAIT:
            if (this.bred) {
                this.overlay.setText(window.language.get(this.LANG_CROSSBREED));
            ...
            if (koi.constellation.small.fish.length > 2) {
                this.overlay.setText(window.language.get(this.LANG_BREED_TOO_MANY));
...
export default TutorialBreeding