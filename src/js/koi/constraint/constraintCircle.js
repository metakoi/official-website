...
import ConstraintCirclePosition from "./constraintCirclePosition";
 ...
 * @constructor
 */
const ConstraintCircle = function(position, width,height) {
    this.position = position;
    this.radius = Math.max(width,height);
    this.width = width-this.MIN;
    this.height = height-this.MIN;
    Constraint.call(this);
};

ConstraintCircle.prototype = Object.create(Constraint.prototype);
ConstraintCircle.prototype.DEPTH = 1.3;
ConstraintCircle.prototype.MIN = 0;
/**
...
ConstraintCircle.prototype.getMeshSteps = function() {
    return Math.ceil(2 * (this.width+this.height) / this.MESH_RESOLUTION);
};
...
ConstraintCircle.prototype.appendMeshWater = function(vertices, indices) {
    ...
    let widthStep = this.width /  this.MESH_RESOLUTION
    let heightStep = this.height /  this.MESH_RESOLUTION
    vertices.push(this.position.x, this.position.y);

    for (let step = 0; step < steps; ++step) {
        let x,y
        if(step <= widthStep){
            x=step*this.MESH_RESOLUTION
            y = this.MIN
        }else if(step <= widthStep+heightStep){
            x= this.width
            y = (step-(widthStep))*this.MESH_RESOLUTION
        }else if(step <= widthStep+heightStep+widthStep){
            x = this.width-((step-(widthStep+heightStep))*this.MESH_RESOLUTION)
            y = this.height
        }else{
            x = this.MIN;
            y = this.height-((step-(widthStep+heightStep+widthStep))*this.MESH_RESOLUTION)
        }
