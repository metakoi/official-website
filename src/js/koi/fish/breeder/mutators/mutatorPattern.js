import LayerRidge from "../../pattern/layer/layerRidge";
import LayerSpots from "../../pattern/layer/layerSpots";
import LayerStripes from "../../pattern/layer/layerStripes";
import LayerWeb from "../../pattern/layer/layerWeb";
import MutatorLayerBase from "./layers/mutatorLayerBase";
import MutatorLayerRidge from "./layers/mutatorLayerRidge";
import MutatorLayerShapeBody from "./layers/mutatorLayerShapeBody";
import MutatorLayerShapeFin from "./layers/mutatorLayerShapeFin";
import MutatorLayerSpots from "./layers/mutatorLayerSpots";
import MutatorLayerStripes from "./layers/mutatorLayerStripes";
import MutatorLayerWeb from "./layers/mutatorLayerWeb";
...
MutatorPattern.prototype.LAYER_COLOR_SWAP_CHANCE = .05;
 ...
 * @param {Random} random A randomizer
 */
MutatorPattern.prototype.mutateColorSwaps = function(random) {
    for (let layer = 0, layers = this.pattern.layers.length; layer < layers; ++layer) {
        if (random.getFloat() < this.LAYER_COLOR_SWAP_CHANCE) {
            const previous = layer === 0 ? this.pattern.base : this.pattern.layers[layer - 1];
            const current = this.pattern.layers[layer];
            const buffer = previous.paletteIndex;

            previous.paletteIndex = current.paletteIndex;
            current.paletteIndex = buffer;
        }
    }
};
...
MutatorPattern.prototype.mutate = function(random) {
    ...
    this.mutateColorSwaps(random);
};

export default MutatorPattern