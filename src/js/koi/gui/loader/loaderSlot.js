...
LoaderSlot.prototype.createButtonContinue = function() {
    ...
    element.innerText = window.language.get(this.LANG_CONTINUE);
    element.onclick = () => {
...
LoaderSlot.prototype.createButtonNew = function() {
    ...
    element.innerText = window.language.get(this.LANG_NEW);
    ...
    element.onclick = () => {
        if (!this.onContinue || element.classList.contains(this.CLASS_BUTTON_CONFIRM))
            ...
            element.innerText = window.language.get(this.LANG_CONFIRM);
        }
    };
